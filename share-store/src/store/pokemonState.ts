...
const createPokemonState: StateCreatorEnhancer<PokemonState> = (set, get) => ({
  pokemons: [],
  select: undefined,
  setPokemons: nextState =>
    set({ pokemons: getNextState(nextState, get().pokemons) }, undefined, {
      type: ACTION.SET_POKEMONS
    }),
  setSelect: nextState =>
    set({ select: getNextState(nextState, get().select) }, undefined, { type: ACTION.SET_SELECT })
});
