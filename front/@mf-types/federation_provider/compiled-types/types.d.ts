...
type SetterKeys<State, Key extends keyof State = keyof State> = Key extends `set${infer U}` ? `set${U}` : never;
...
type StateCreatorEnhancer<State> = StateCreator<State, [
    ['zustand/immer', never],
    ['zustand/devtools', never]
], [
], State>;
export type { State, SetterKeys, PropsSelector, Setter, StateHooks, StateHookCreator, StateCreatorEnhancer };
