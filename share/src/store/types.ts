...
type Setter<State> = {
  [Property in keyof State as `set${Capitalize<Lowercase<Property & string>>}`]: (
    nextState: State[Property] | StateUpdater<State[Property]>
  ) => void;
};
...
type StateHookCreator<State, Store extends StoreApi<State> = StoreApi<State>> = (
  store: Store
) => StateHooks<State>;

type StateCreatorEnhancer<State> = StateCreator<
  State,
