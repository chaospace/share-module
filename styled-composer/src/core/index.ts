...
const getKeyValue = (key: string | number, provider: StyleValueType) =>
  getOrReplaceValue(key, provider, key);

const isStyleProcessorConfig = (
  b: StyleProcessorParameter | boolean
): b is StyleProcessorParameter => typeof b !== 'boolean';

const createStyleProcessor = ({
  property,
  alias,
  interpreter = getKeyValue,
  defaultAlias
}: StyleProcessorParameter): StyleProcessor => {
...
export type { StyleComposer, StyleStateKeyWord, CSSComposerObject };
export {
  createStyleProcessor,
